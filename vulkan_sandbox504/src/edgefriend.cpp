...
EdgefriendGeometry SubdivideToEdgefriendGeometry(
    ...
    std::unordered_map<glm::ivec2, float> oldCreases) {
    ...
    std::vector<unsigned long long> faceView(oF);
    std::iota(faceView.begin(), faceView.end(), 0ull);
    ...
    std::vector<unsigned long long> vertexView(oV);
    std::iota(vertexView.begin(), vertexView.end(), 0ull);
    ...
    EdgefriendGeometry result;
    result.positions = std::move(newPositions);
    result.indices = std::move(newIndices);
    result.friendsAndSharpnesses = std::move(newFriendsAndSharpnesses);
    result.valenceStartInfos = std::move(newValenceStartInfos);
...
template <typename T>
glm::uint32_t Load(const std::vector<T> &buffer, std::uint32_t address) {
    auto bytes = reinterpret_cast<const std::byte *>(buffer.data());
    return *reinterpret_cast<const std::uint32_t *>(bytes + address);
}

template <typename T>
glm::uvec2 Load2(const std::vector<T> &buffer, std::uint32_t address) {
    auto bytes = reinterpret_cast<const std::byte *>(buffer.data());
...
template <typename T>
glm::uvec3 Load3(const std::vector<T> &buffer, std::uint32_t address) {
    auto bytes = reinterpret_cast<const std::byte *>(buffer.data());
...
template <typename T>
glm::uvec4 Load4(const std::vector<T> &buffer, std::uint32_t address) {
    auto bytes = reinterpret_cast<const std::byte *>(buffer.data());
...
template <typename T>
void Store(std::vector<T> &buffer, std::uint32_t address, const glm::uint32_t value) {
    auto bytes = reinterpret_cast<std::byte *>(buffer.data());
    *reinterpret_cast<std::uint32_t *>(bytes + address) = value;
}

template <typename T>
void Store2(std::vector<T> &buffer, std::uint32_t address, const glm::uvec2 &values) {
    auto bytes = reinterpret_cast<std::byte *>(buffer.data());
...
template <typename T>
void Store3(std::vector<T> &buffer, std::uint32_t address, const glm::uvec3 &values) {
    auto bytes = reinterpret_cast<std::byte *>(buffer.data());
    *reinterpret_cast<std::uint32_t *>(bytes + address + 0) = values[0];
    *reinterpret_cast<std::uint32_t *>(bytes + address + 4) = values[1];
...
template <typename T>
void Store4(std::vector<T> &buffer, std::uint32_t address, const glm::uvec4 &values) {
    auto bytes = reinterpret_cast<std::byte *>(buffer.data());
    *reinterpret_cast<std::uint32_t *>(bytes + address + 0) = values[0];
    *reinterpret_cast<std::uint32_t *>(bytes + address + 4) = values[1];
    *reinterpret_cast<std::uint32_t *>(bytes + address + 8) = values[2];
...
void CSEdgefriendIter(uint3 dispatchThreadID, const EdgefriendGeometry &old, EdgefriendGeometry &neu) {
...
EdgefriendGeometry SubdivideEdgefriendGeometry(const EdgefriendGeometry &old) {
    ...
    std::vector<unsigned long long> threadView(oV);
    std::iota(threadView.begin(), threadView.end(), 0ull);
    std::for_each(EXECUTION_POLICY, threadView.begin(), threadView.end(), [&](auto thread) {
        CSEdgefriendIter(glm::uvec3(thread, 0, 0), old, neu);
    });
    return neu;
}